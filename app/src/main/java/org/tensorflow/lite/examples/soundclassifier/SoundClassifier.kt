...
class SoundClassifier(
    ...
    fun runMetaInterpreter(location: Location) {
        ...
        Handler(Looper.getMainLooper()).post {
            mBinding.gps.setText(
                mContext.getString(R.string.latitude) + ": " + (round(lat * 100.0) / 100.0).toString() + " / " + mContext.getString(
                    R.string.longitude
                ) + ": " + (round(lon * 100.0) / 100).toString()
            )
        }
        ...
        recognitionTask = Timer().scheduleAtFixedRate(inferenceInterval, inferenceInterval) task@{
            ...
            if (samplesAreAllZero) {
                ...
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(
                        mContext,
                        mContext.resources.getString(R.string.samples_zero),
                        Toast.LENGTH_SHORT
                    ).show()
                }
            ...
            if (mBinding.progressHorizontal.isIndeterminate) {  //if start/stop button set to "running"
                probList.withIndex().also {
                    ...
                    val secondMax =
                        it.filterNot { entry -> entry == max }.maxByOrNull { entry -> entry.value }
